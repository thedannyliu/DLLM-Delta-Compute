...
python -m lm_eval \
    --model diffllm \
    --model_args pretrained=Dream-org/Dream-v0-Instruct-7B,delta_mode=p0_baseline,cache_mode=phaseB_router,router_checkpoint=${ROUTER_CKPT},cache_warmup=5 \
    --tasks gsm8k \
    --num_fewshot 5 \
    --batch_size 1 \
    --limit 500 \
    --output_path results/PhaseB_500_$(date +%Y%m%d_%H%M%S)
