...
python -m lm_eval \
    --model diffllm \
    --model_args pretrained=Dream-org/Dream-v0-Instruct-7B,delta_mode=p2_earlystop,cache_mode=none,early_stop_norm_threshold=0.01 \
    --tasks gsm8k \
    --num_fewshot 5 \
    --batch_size 1 \
    --limit 500 \
    --output_path results/P2_500_$(date +%Y%m%d_%H%M%S)
