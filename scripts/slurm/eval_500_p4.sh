...
python -m lm_eval \
    --model diffllm \
    --model_args pretrained=Dream-org/Dream-v0-Instruct-7B,delta_mode=p4_adaptive,cache_mode=none,adaptive_max_stride=4,adaptive_lte_threshold=0.01,adaptive_entropy_threshold=2.0 \
    --tasks gsm8k \
    --num_fewshot 5 \
    --batch_size 1 \
    --limit 500 \
    --output_path results/P4_500_$(date +%Y%m%d_%H%M%S)
